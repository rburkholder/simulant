...
void WaveformView::HandlePaint( wxPaintEvent& event ) {
  ...
  if ( 0 != m_pvSamples ) {
    if ( 0 != m_pvSamples->size() ) {
      ...
      int dxStep = m_pvSamples->size() / ( 1 + width ); // the one for getting last value on display, check if correct for #samples < width
      int ixStep = 0;
      int halfheight = height / 2;
      int scale = std::numeric_limits<short>::max() / halfheight;
      int xPrv, yPrv;
      for ( int ix = 0; ix < width; ++ix ) {
        int val = ( (*m_pvSamples)[ ixStep ] / scale ) + halfheight;
        if ( 0 == ix ) {
          xPrv = 0;
